...
export default function BrowsePage() {
  ...
  useEffect(() => {
    mockApi.resources.listTags().then(setAllTags);
  }, []);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    mockApi.resources
      .list({ type, q, tags: selectedTags, sort, status: "published" })
      .then((res) => {
        if (!alive) return;
        setItems(res);
      })
  ...
  }, [type, q, selectedTags, sort]);

  const typeTabs: { key: TypeFilter; label: string }[] = useMemo(
    () => [
      { key: "all", label: "全部" },
      { key: "map", label: "地图" },
      { key: "mod", label: "模组" },
      { key: "taskpack", label: "任务包" },
      { key: "plugin", label: "插件" }
    ],
    []
  );
    ...
    <AppShell>
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="text-xl font-semibold">浏览资源</div>
            <div className="mt-1 text-sm text-muted-foreground">按类型与标签发现资源，点击进入详情页讨论。</div>
          </div>
